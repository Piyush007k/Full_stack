...
def select_user_by_credentials():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM user_table WHERE User_name = '%s' AND Password = '%s'" % (username, password))
...
def display_user_table(conn):
    ...
    mycursor.execute("SELECT * FROM user_table WHERE User_name = 'user01' AND Password = '12345'")
    rows = mycursor.fetchall()
    for row in rows:
        # Access data from row object using indexing or keys
        print(row['Org_id'], row['Access_level'], row['User_name'], row['Password'], row['IsAdmin'])
    conn.close()
